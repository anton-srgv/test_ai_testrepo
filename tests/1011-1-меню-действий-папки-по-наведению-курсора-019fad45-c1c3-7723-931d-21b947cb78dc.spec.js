...
/**
 * @externalId 2064
 * @recipeId 019fad45-c1c3-7723-931d-21b947cb78dc
 * @title 1011-1 Меню действий папки по наведению курсора
 * @description &lt;p&gt;Нормализовано из #1011. Предусловия: [заранее] [API] В «Пространство_1» существует «Папка_1»: POST &#x2F;cwm&#x2F;public&#x2F;api&#x2F;v1&#x2F;workspaces&#x2F;TESTAI&#x2F;folders; [заранее] [API] Учётная запись ts_login — участник «Пространство_1» с ролью пространства Admin&lt;&#x2F;p&gt;
 */
test(
  '1011-1 Меню действий папки по наведению курсора',
  { tag: ['@normalized', '@src-1011'] },
  async ({ page }) => {
    testit.workItemIds(['2064']);
    ...
    // Step 5: Открыть /tasks/tasks.
    // Expected result: открыт раздел «Задачи», в дереве отображается «Пространство_1».
    // Action: Navigate to /tasks/tasks
    {
      let apiTriggered = false;
      const pendingApiRequests = new Set();
      const isApiRequest = (req) => {
      ...
      try {
        {
          const prevContainerScrollDimension = await page
            .locator('a[href="/tasks/tasks/my-tickets"]')
          ...
          try {
            const locator = page.locator('a[href="/tasks/tasks/my-tickets"]');
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "[data-guide-element='tasksLayout_explorerSelect'] app-indicator-opened",
              )
              .click();
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[role='option']:has-text('Пространство_1')")
              .click();
    ...
    // Assertion: Открыт раздел «Задачи» — URL содержит /tasks/tasks
    await expect(page).toHaveURL(new RegExp('\\/tasks\\/tasks'));
    // Assertion: В дереве левой панели отображается «Пространство_1»
    await expect(
      page.locator(
        "[data-testid='TasksExplorerSidebar'] app-workspace-selector:has-text('Пространство_1')",
      ),
    ).toBeVisible();

    // Step 6: Нажать «Пространство_1» в дереве.
    // Expected result: под названием «Пространство_1» отображается «Папка_1».
    // Assertion: Под названием «Пространство_1» отображается «Папка_1» в дереве навигации
    await expect(
      page.locator("[data-testid='TasksExplorerTreeNode']:has-text('Папка_1')"),
    ).toBeVisible();

    // Step 7: Навести курсор на название «Папка_1».
    // Expected result: рядом с названием «Папка_1» отображается кнопка дополнительных действий (многоточие).
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "[data-testid='TasksExplorerTreeNode']:has-text('Папка_1')",
              )
              .hover();
    ...
    // Assertion: Рядом с названием «Папка_1» отображается кнопка дополнительных действий (многоточие) после наведения курсора
    await expect(
      page.locator(
        "app-tree-node-layout:has-text('Папка_1') [data-testid='kebabButton']",
      ),
    ).toBeVisible();

    // Step 8: Нажать кнопку дополнительных действий у «Папка_1».
    // Expected result: открыто меню с пунктом «Настройки».
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "app-folder-tree-node [data-testid='kebabButton'] button",
              )
              .click();
    ...
    // Assertion: В открывшемся меню отображается пункт «Настройки»
    await expect(
      page.locator("[data-testid='kebabPopupSettingsOption']"),
    ).toBeVisible();

    // Step 9: Выбрать «Настройки».
    // Expected result: открыта страница настроек «Папка_1».
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[data-testid='kebabPopupSettingsOption']")
              .click();
    ...
    // Assertion: URL содержит '/folder-settings/', подтверждая открытие страницы настроек папки
    await expect(page).toHaveURL(new RegExp('\\/folder-settings\\/'));
    // Assertion: Заголовок страницы содержит 'Папка_1', подтверждая что открыты настройки именно этой папки
    await expect(page).toHaveTitle(new RegExp('Папка_1'));
    // Assertion: На странице отображается заголовок 'Настройки папки'
    await expect(page.locator("text='Настройки папки'")).toBeVisible();
