...
/**
 * @externalId 335
 * @recipeId 019ed4f9-852c-73ce-a566-d5bf86ad6149
 * @title Toggle secret для разных типов env data имеет корректное состояние
 * @description Automatically generated test
 */
test(
  'Toggle secret для разных типов env data имеет корректное состояние',
  { tag: ['@regression'] },
  async ({ page }) => {
    testit.workItemIds(['335']);
    ...
    // Assertion: Пользователь авторизован — произошёл редирект на страницу проекта
    await expect(page).toHaveURL(new RegExp('\\/recipes'));
    // Assertion: Отображается страница 'Default Project' — заголовок виден в левой панели
    await expect(page.locator("text='Default Project'")).toBeVisible();
    // Assertion: В верхней части панели отображается кнопка с иконкой папки и названием проекта
    await expect(
      page.locator(
        "[data-testid='recipes-list-sections-tree-item-button-root']",
      ),
    ).toBeVisible();
    // Assertion: В нижней части панели отображается аватар с первой буквой email и email пользователя
    await expect(
    ...
    // Assertion: URL содержит /settings/environment — подтверждает переход на страницу настроек окружения (env Data)
    await expect(page).toHaveURL(new RegExp('\\/settings\\/environment'));
    // Assertion: Отображается заголовок 'Настройки окружения', подтверждающий открытие страницы редактирования env Data
    await expect(
      page.locator("[data-testid='environment-page-page'] > span"),
    ).toBeVisible();
    // Assertion: Отображается секция 'Данные окружения' — основная часть страницы редактирования env Data
    await expect(page.locator('#env-data-heading')).toBeVisible();

    // Step 6: Нажать на кнопку: “Add“
    // Expected result: Открылось окно для добавления новой Test Data в проект
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
    ...
    // Assertion: Открылось модальное окно для добавления новой Test Data в проект с заголовком 'Новые тестовые данные'
    await expect(
      page.locator("[data-testid='env-data-dialog-dialog']"),
    ).toBeVisible();

    // Step 7: Выбрать data type login
    // Expected result: toggle secret - off
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[data-testid='env-data-dialog-dropdown-item-login']")
              .click();
    ...
    // Assertion: Переключатель 'Секретное значение' находится в выключенном состоянии (off) после выбора типа данных 'Логин'
    await expect(
      page.locator("[data-testid='env-data-dialog-checkbox-is-secret']"),
    ).toHaveAttribute('data-state', 'unchecked');

    // Step 8: Выбрать data type password
    // Expected result: toggle secret - on
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[data-testid='env-data-dialog-dropdown-item-password']")
              .click();
    ...
    // Assertion: Toggle 'Секретное значение' включён (on) после выбора типа данных 'Пароль'
    await expect(
      page.locator("[data-testid='env-data-dialog-checkbox-is-secret']"),
    ).toHaveAttribute('data-state', 'checked');

    // Step 9: Выбрать data type Api Key
    // Expected result: toggle secret - on
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[data-testid='env-data-dialog-dropdown-item-api_key']")
              .click();
    ...
    // Assertion: Toggle 'Секретное значение' включён (on) после выбора типа данных API ключ
    await expect(
      page.locator("[data-testid='env-data-dialog-checkbox-is-secret']"),
    ).toHaveAttribute('data-state', 'checked');

    // Step 10: Выбрать data type Other
    // Expected result: toggle secret - off
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
    ...
    // Assertion: Toggle 'Секретное значение' выключен (off) после выбора типа данных 'Другое'
    await expect(
      page.locator("[data-testid='env-data-dialog-checkbox-is-secret']"),
    ).toHaveAttribute('data-state', 'unchecked');
