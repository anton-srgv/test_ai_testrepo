...
/**
 * @externalId 549
 * @recipeId 019ed4f9-8592-7608-b02c-9f4fdf3b72dd
 * @title Успешное добавление SUT URL в Env Data
 * @description Automatically generated test
 */
test(
  'Успешное добавление SUT URL в Env Data',
  { tag: ['@regression'] },
  async ({ page }) => {
    testit.workItemIds(['549']);
    ...
    await expect(
      page.locator("aside span.font-bold:has-text('Default Project')"),
    ).toBeVisible();
    ...
    // Assertion: URL подтверждает переход на страницу настроек окружения
    ...
    // Step 6: В поле SUT URL ввести корректное значение, нужно генерить уникальное на каждый запуск автотеста

    const uniqueSutUrl = faker.helpers.fake(
    ...
    // Step 7: Нажать кнопку сохранить
    // Expected result: SUT URL поле отображается с измененным значением; Появилось уведомление: * заголовок - “Настройки сохранены“; * текст - “Настройки проекта успешно обновлены“;
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[data-testid='project-settings-button-save']")
              .click();
    ...
    // Assertion: SUT URL поле отображается с измененным значением (содержит введённый уникальный URL)
    await expect(
      page.locator("[data-testid='project-settings-input-sut-url']"),
    ).toHaveValue(uniqueSutUrl);
