...
/**
 * @externalId 682
 * @recipeId 019ed4f9-8583-708e-802c-917592ecfc01
 * @title Неуспешное редактирование SUT URL
 * @description Automatically generated test
 */
test(
  'Неуспешное редактирование SUT URL',
  { tag: ['@regression', '@smoke'] },
  async ({ page }) => {
    testit.workItemIds(['682']);
    ...
    // Assertion: Отображается страница 'Default Project'
    await expect(
      page.locator(
        "[data-testid='sidebar-layout-container'] aside span:has-text('Default Project')",
      ),
    ).toBeVisible();
    // Assertion: В верхней части левой панели навигации отображается кнопка с иконкой папки и названием проекта
    await expect(
      page.locator(
        "[data-testid='recipes-list-sections-tree-item-button-root']",
      ),
    ).toBeVisible();
    // Assertion: В нижней части левой панели навигации отображается аватар с первой буквой email и email пользователя
    await expect(
    ...
    // Assertion: URL содержит /settings/environment — подтверждает переход на страницу настроек окружения
    await expect(page).toHaveURL(new RegExp('\\/settings\\/environment'));
    // Assertion: Отображается заголовок 'Настройки окружения', подтверждающий что открылась страница редактирования env Data
    await expect(
      page.locator("[data-testid='environment-page-page'] > span"),
    ).toBeVisible();

    // Step 6: Изменить SUT URL на невалидное значение
    // Expected result: Валидация с красной подсветкой поля sut url с текстом: “Please enter a valid URL (e.g., https://example.com [https://example.com])“; Кнопка: “Save“ в состоянии disabled;
    const invalidSutUrl = 'not-a-valid-url1';
    // Action: Input variable value into element with data-attribute
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await fillInput(
              page,
              "[data-testid='project-settings-input-sut-url']",
              invalidSutUrl,
            );
    ...
    // Assertion: Отображается текст валидации 'Please enter a valid URL (e.g., https://example.com)' под полем SUT URL
    await expect(
      page.locator("[data-testid='project-settings-error-sut-url']"),
    ).toContainText('Пожалуйста, введите корректный URL');
    // Assertion: Поле SUT URL имеет красную подсветку (состояние ошибки валидации)
    await expect(
      page.locator("[data-testid='project-settings-error-sut-url']"),
    ).toBeVisible();
    // Assertion: Кнопка 'Save' (Сохранить) находится в состоянии disabled
    await expect(
      page.locator("[data-testid='project-settings-button-save']"),
    ).toBeDisabled();
