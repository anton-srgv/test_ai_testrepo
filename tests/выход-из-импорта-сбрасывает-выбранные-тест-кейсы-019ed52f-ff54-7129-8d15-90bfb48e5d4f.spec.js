...
/**
 * @externalId 1450
 * @recipeId 019ed52f-ff54-7129-8d15-90bfb48e5d4f
 * @title Выход из импорта сбрасывает выбранные тест-кейсы
 * @description Automatically generated test
 */
test(
  'Выход из импорта сбрасывает выбранные тест-кейсы',
  { tag: ['@blocker', '@regression', '@smoke', '@to-automate'] },
  async ({ page }) => {
    testit.workItemIds(['1450']);
    ...
    // Assertion: Пользователь авторизован — URL изменился на страницу рецептов проекта
    await expect(page).toHaveURL(new RegExp('\\/recipes'));
    // Assertion: Отображается страница 'Default Project' — заголовок проекта виден в левой панели
    await expect(
      page.locator(
    ...
    await expect(
      page.locator("h1:has-text('Импортировать из Test')"),
    ).toBeVisible();
    // Assertion: Кнопка 'Сбросить' видна на странице
    await expect(
    ...
    // Assertion: Кнопка 'Импортировать' видна на странице
    await expect(
    ...
    // Assertion: Дропдаун с выбором проекта отображается (видна иконка раскрытия — значит проектов несколько)
    await expect(
      ...
      try {
        {
          ...
          try {
            await page
              .locator("tr:has-text('376') button[role='checkbox']")
              .click();
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "tr:has(td:nth-child(2):has-text('1152')) button[role='checkbox']",
              )
              .click();
    ...
    // Step 7: Выйти из импорта на страницу рецептов
    // Expected result: Выход с экрана импорта (кнопка «←» или навигация) сбрасывает выбор без предупреждения
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[data-testid='import-page-top-bar-button-cancel']")
              .click();
    ...
    // Assertion: Пользователь перешёл на страницу рецептов — выход с экрана импорта произошёл успешно без предупреждения
    await expect(page).toHaveURL(new RegExp('\\/recipes'));
    // Assertion: Отображается страница рецептов с кнопкой «Импортировать», подтверждая что выход с экрана импорта произошёл и мы на странице рецептов
    await expect(
      page.locator("[data-testid='recipes-import-button-import']"),
    ).toBeVisible();
