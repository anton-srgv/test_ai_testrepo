...
/**
 * @externalId 2062
 * @recipeId 019fad45-c1ad-767e-a459-5c5b89953bcf
 * @title 355-2 Раздел «Лицензии» отображает счётчики пользователей и гостей
 * @description &lt;p&gt;Нормализовано из #355. Предусловия: [заранее] Учётная запись ts_login имеет роль инстанса «Системный Администратор»&lt;&#x2F;p&gt;
 */
test(
  '355-2 Раздел Лицензии отображает счтчики пользователей и гостей',
  { tag: ['@normalized', '@src-355'] },
  async ({ page }) => {
    testit.workItemIds(['2062']);
    ...
    // Assertion: Открыта страница «Авторизация» — URL содержит /login
    await expect(page).toHaveURL(new RegExp('\\/login'));
    // Assertion: Заголовок страницы содержит «Авторизация»
    ...
    await expect(page.locator('text=Пространство_1')).toBeVisible();

    // Step 5: Открыть /admin/admin-settings/licenses.
    // Expected result: открыт раздел «Лицензии», отображаются «Идентификатор сервера», строка «Активных пользователей», строка «Гостей», статус лицензии и срок её действия.
    // Action: Navigate to /admin/admin-settings/licenses
    {
      let apiTriggered = false;
      const pendingApiRequests = new Set();
      const isApiRequest = (req) => {
      ...
      try {
        await page.goto('/admin/admin-settings/licenses');
    ...
    // Assertion: Открыт раздел «Лицензии» — URL соответствует ожидаемому
    await expect(page).toHaveURL(
      new RegExp('\\/admin\\/admin-settings\\/licenses'),
    );
    // Assertion: Отображается заголовок раздела «Лицензии»
    await expect(
      page.locator("[data-testid='licenseScreen'] span[headercontent]"),
    ).toHaveText('Лицензии');
    // Assertion: Отображается «Идентификатор сервера»
    await expect(
      page.locator("[data-testid='licenseScreen'] .server-id"),
    ).toContainText('Идентификатор сервера');
    // Assertion: Отображается строка «Активных пользователей» с числовым значением
    await expect(
      page.locator("[data-testid='licenseScreen'] .users-size"),
    ).toContainText(new RegExp('Активных пользователей.*\\d+/\\d+'));
    // Assertion: Отображается строка «Гостей» с числовым значением
    await expect(
      page.locator("[data-testid='licenseScreen'] .guests-size"),
    ).toContainText(new RegExp('Гостей.*\\d+/\\d+'));
    // Assertion: Отображается статус лицензии (например, «Пробная версия продукта»)
    await expect(page.locator("[data-testid='trialTitle']")).toBeVisible();
    // Assertion: Отображается срок действия лицензии с датой
    await expect(
      page.locator('app-trial-zero-state .description'),
    ).toContainText(new RegExp('Действительна до \\d{2}\\.\\d{2}\\.\\d{4}'));
