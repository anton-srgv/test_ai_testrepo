...
/**
 * @externalId 1449
 * @recipeId 019ed52f-ff5e-71a2-8800-34e34cb27a0a
 * @title Смена проекта сбрасывает выбранные тест-кейсы
 * @description Automatically generated test
 */
test(
  'Смена проекта сбрасывает выбранные тест-кейсы',
  { tag: ['@blocker', '@regression', '@smoke', '@to-automate'] },
  async ({ page }) => {
    testit.workItemIds(['1449']);
    ...
    // Assertion: Пользователь авторизован — URL изменился на страницу приложения
    await expect(page).toHaveURL(new RegExp('\\/recipes'));
    // Assertion: Отображается страница 'Default Project' — заголовок виден
    await expect(
      page.locator("aside span:has-text('Default Project')"),
    ).toBeVisible();
    // Assertion: В верхней части навигации отображается кнопка с иконкой папки и названием проекта
    await expect(
      page.locator(
        "[data-testid='recipes-list-sections-tree-item-button-root']",
      ),
    ).toBeVisible();
    // Assertion: В нижней части навигации отображается аватар с первой буквой email и email пользователя
    await expect(
      page.locator("[data-testid='sidebar-button-user']"),
    ).toBeVisible();

    // Step 5: На странице рецептов нажать: “Импортировать”
    // Expected result: Открылась страница: /import Заголовок: Импортировать из TestIT Кнопки: * «Сбросить» всегда видна, disabled при счетчике = 0; * «Импортировать» всегда видна, disabled при счетчике = 0; Сверху над секциями тест-кейсов есть дропдаун с выбором проекта: * Если проект в интеграции один – дропдаун скрыт, только название * Если проектов несколько – дропдаун, по умолчанию первый согласно порядку TMS API [/api/v2/Attachments/019e43fa-93c0-740e-b83c-292aaca7ab0b]
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[data-testid='recipes-import-button-import']")
              .click();
    ...
    // Assertion: Открылась страница с URL /import
    await expect(page).toHaveURL(new RegExp('\\/import'));
    // Assertion: Заголовок страницы содержит текст 'Импортировать из TestIT'
    await expect(
      page.locator("h1:has-text('Импортировать из Test IT')"),
    ).toBeVisible();
    // Assertion: Кнопка 'Сбросить' всегда видна на странице импорта
    await expect(
      page.locator("[data-testid='import-page-top-bar-button-reset']"),
    ).toBeVisible();
    // Assertion: Кнопка 'Сбросить' задизейблена при счётчике = 0
    await expect(
      page.locator("[data-testid='import-page-top-bar-button-reset']"),
    ).toBeDisabled();
    // Assertion: Кнопка 'Импортировать' всегда видна на странице импорта
    await expect(
      page.locator("[data-testid='import-page-top-bar-button-import']"),
    ).toBeVisible();
    // Assertion: Кнопка 'Импортировать' задизейблена при счётчике = 0
    await expect(
      page.locator("[data-testid='import-page-top-bar-button-import']"),
    ).toBeDisabled();
    // Assertion: Дропдаун с выбором проекта отображается над секциями тест-кейсов
    await expect(
      page.locator("[data-testid='import-page-sidebar-dropdown']"),
    ).toBeVisible();

    // Step 6: Выбрать несколько тест-кейсов по чек-боксу
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[data-testid='import-page-sidebar-dropdown']")
              .click();
      ...
      try {
        {
          ...
          try {
            await page.locator("[data-testid='import-page-header'] h1").click();
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "tr:has(td:nth-child(2):has-text('376')) button[role='checkbox']",
              )
              .click();
      ...
      try {
        {
          ...
          try {
            await page
              .locator("tr:has-text('956') button[role='checkbox']")
              .click();
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "[data-testid='import-page-table-section'] tr:has-text('1152') button[role='checkbox']",
              )
              .click();
    ...
    // Step 7: Сменить проект в дропдауне
    // Expected result: Смена проекта сбрасывает дерево, таблицу, счётчик и выбор, уведомление пользователя: Проект переключён Выбранные тест-кейсы сброшены
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
    ...
    // Action: Click on element with aria-role locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "[role='menu'] [role='menuitem']:has-text('Pilot - tot pop')",
              )
              .click();
    ...
    // Assertion: Дропдаун проекта отображает новый выбранный проект 'Pilot - tot pop', подтверждая смену проекта
    await expect(
      page.locator("[data-testid='import-page-sidebar-dropdown']"),
    ).toContainText('Pilot - tot pop');
    // Assertion: Кнопка 'Сбросить' задизейблена, что подтверждает сброс счётчика выбранных тест-кейсов до 0
    await expect(
      page.locator("[data-testid='import-page-top-bar-button-reset']"),
    ).toBeDisabled();
