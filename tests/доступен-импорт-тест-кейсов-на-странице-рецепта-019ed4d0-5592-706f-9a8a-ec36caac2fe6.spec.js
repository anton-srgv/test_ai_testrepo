...
/**
 * @externalId 533
 * @recipeId 019ed4d0-5592-706f-9a8a-ec36caac2fe6
 * @title Доступен импорт тест-кейсов на странице рецепта
 * @description Automatically generated test
 */
test(
  'Доступен импорт тест-кейсов на странице рецепта',
  { tag: ['@regression', '@to-automate'] },
  async ({ page }) => {
    testit.workItemIds(['533']);
    ...
    // Assertion: Форма авторизации отображается — кнопка «Войти» видима
    await expect(
    ...
    // Assertion: В нижней части левой панели навигации доступна кнопка TMS Connections
    await expect(
      page.locator("[data-testid='sidebar-nav-item-settings']"),
    ).toBeVisible();
    // Assertion: В нижней части левой панели навигации доступна кнопка Members
    await expect(
    ...
    // Step 5: Нажать кнопку:Import
    // Expected result: доступны действия: * Открылась страница с импортом из TestIt
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
    ...
    // Assertion: URL содержит /import — подтверждает переход на страницу импорта
    await expect(page).toHaveURL(new RegExp('\\/import'));
    // Assertion: На странице отображается заголовок с упоминанием Test IT — подтверждает, что это страница импорта из TestIt
    await expect(page.locator("h1:has-text('Test IT')")).toBeVisible();
