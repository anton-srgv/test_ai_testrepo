...
/**
 * @externalId 336
 * @recipeId 019ed4f9-84eb-766d-94f0-105481b3a411
...
test(
  ...
  async ({ page }) => {
    ...
    await expect(
      page.locator(
        "aside span.font-golos.font-bold:has-text('Default Project')",
      ),
    ).toBeVisible();
    ...
    // Assertion: URL страницы содержит /settings/environment — подтверждает переход на страницу настроек окружения
    ...
    // Assertion: Отображается секция 'Данные окружения' — подтверждает, что страница содержит env Data для редактирования
    await expect(page.locator('#env-data-heading')).toBeVisible();

    // Step 6: Изменить SUT URL

    const newSutUrl = faker.helpers.fake(
