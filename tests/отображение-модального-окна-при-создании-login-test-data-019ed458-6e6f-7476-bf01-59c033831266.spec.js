...
/**
 * @externalId 343
 * @recipeId 019ed458-6e6f-7476-bf01-59c033831266
 * @title Отображение модального окна при создании Login test data
 * @description Automatically generated test
 */
test(
  'Отображение модального окна при создании Login test data',
  { tag: ['@regression', '@smoke'] },
  async ({ page }) => {
    testit.workItemIds(['343']);
    ...
    // Assertion: Открылась страница настроек окружения (Environment Data)
    await expect(page).toHaveURL(new RegExp('\\/settings\\/environment'));
    // Assertion: На странице отображается блок 'Данные окружения' (Environment Data), подтверждающий что это страница редактирования env Data
    ...
    // Assertion: Открылось модальное окно с заголовком 'Новые тестовые данные' (соответствует 'New test data')
    await expect(
      page.locator("[data-testid='env-data-dialog-dialog']"),
    ).toBeVisible();

    // Step 7: В дропдауне “Data Type“ выбрать “Log In“
    // Expected result: В модальном окне отображаются следующие элементы: 1. Поле “Name” внутри которого: “e.g., USERNAME”, под полем описание: “Login for system access” 2. Дропдаун: “Data type” с выбранным вариантом: “Login” 3. Поле “Value” внутри которого пример: “admin@example.com [admin@example.com]” 4. Toggle: “Secret value” с состоянием по дефолту: off 5. Кнопки: “Save”, “Cancel”
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
    ...
    // Assertion: Поле 'Name' содержит плейсхолдер 'e.g., USERNAME' (в локализации: 'например, USERNAME')
    await expect(
      page.locator("[data-testid='env-data-dialog-input-key']"),
    ).toHaveAttribute('placeholder', 'например, USERNAME');
    // Assertion: Под полем Name отображается описание 'Login for system access' (в локализации: 'Логин для входа')
    await expect(
      page.locator(
        "[data-testid='env-data-dialog-form'] span:has-text('Логин для входа')",
      ),
    ).toBeVisible();
    // Assertion: Дропдаун 'Data type' отображает выбранный вариант 'Login' (в локализации: 'Логин')
    await expect(
      page.locator("[data-testid='env-data-dialog-button-type']"),
    ).toContainText('Логин');
    // Assertion: Поле 'Value' содержит плейсхолдер 'admin@example.com'
    await expect(
      page.locator("[data-testid='env-data-dialog-input-value']"),
    ).toHaveAttribute('placeholder', 'admin@example.com');
    // Assertion: Toggle 'Secret value' в состоянии off по умолчанию
    await expect(
      page.locator("[data-testid='env-data-dialog-checkbox-is-secret']"),
    ).toHaveAttribute('data-state', 'unchecked');
    // Assertion: Кнопка 'Save' (в локализации: 'Сохранить') отображается в модальном окне
    await expect(
      page.locator("[data-testid='env-data-dialog-dialog-button-save']"),
    ).toBeVisible();
    // Assertion: Кнопка 'Cancel' (в локализации: 'Отмена') отображается в модальном окне
    await expect(
