...
/**
 * @externalId 342
 * @recipeId 019ed458-6e58-7567-859d-ccca99f12d4a
 * @title Отображение модального окна при создании Api Key test data
 * @description Automatically generated test
 */
test(
  'Отображение модального окна при создании Api Key test data',
  { tag: ['@regression', '@smoke'] },
  async ({ page }) => {
    testit.workItemIds(['342']);
    ...
    // Assertion: Пользователь авторизован — произошёл переход со страницы логина на основную страницу
    ...
    await expect(
      page.locator(
        "[data-testid='sidebar-layout-container'] aside span.truncate",
      ),
    ).toBeVisible();
    ...
    // Assertion: Отображается заголовок страницы редактирования Environment Data
    await expect(
      page.locator("[data-testid='environment-page-page'] > span"),
    ).toBeVisible();
    // Assertion: Отображается блок Environment Data (Данные окружения) на странице
    await expect(
      page.locator("[data-testid='env-data-container'] #env-data-heading"),
    ).toBeVisible();

    // Step 6: В блоке “Environment Data“ нажать кнопку “Add”
    // Expected result: Открылось модальное окно с 1. заголовком: “New test data“ 2. Полем: “Name”, под которым описание: “API key for accessing external services” 3. Дропдауном: “Data type” и по дефолту выбором “API Key” 4. Полем “Value” 5. Toggle: “Secret value”, под которым описание: “Won't be visible in the fields“ по дефолту toggle status: on 6. Двумя кнопками: “Save”, “Cancel”
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
    ...
    // Assertion: Модальное окно имеет заголовок 'New test data' (на скриншоте: 'Новые тестовые данные')
    await expect(page.locator("text='Новые тестовые данные'")).toBeVisible();
    // Assertion: Поле 'Name' (Название) отображается в модальном окне
    await expect(
      page.locator(
        "[data-testid='env-data-dialog-dialog'] [data-testid='env-data-dialog-input-key']",
      ),
    ).toBeVisible();
    // Assertion: Под полем Name отображается описание 'API key for accessing external services'
    await expect(
      page.locator(
        "[data-testid='env-data-dialog-form'] span:has-text('API ключ для доступа к внешним сервисам')",
      ),
    ).toBeVisible();
    // Assertion: Дропдаун 'Data type' по умолчанию содержит значение 'API Key' (на скриншоте: 'API ключ')
    await expect(
      page.locator("[data-testid='env-data-dialog-button-type']"),
    ).toContainText('API ключ');
    // Assertion: Поле 'Value' (Значение) отображается в модальном окне
    await expect(
      page.locator(
        "[data-testid='env-data-dialog-dialog'] [data-testid='env-data-dialog-input-value']",
      ),
    ).toBeVisible();
    // Assertion: Toggle 'Secret value' по умолчанию включён (on)
    await expect(
      page.locator("[data-testid='env-data-dialog-checkbox-is-secret']"),
    ).toHaveAttribute('data-state', 'checked');
    // Assertion: Под тоглом отображается описание 'Won't be visible in the fields'
    await expect(
      page.locator(
        "[data-testid='env-data-dialog-form'] label:has-text('Не будет видно в полях')",
      ),
    ).toBeVisible();
    // Assertion: Кнопка 'Save' (Сохранить) отображается в модальном окне
    await expect(
      page.locator("[data-testid='env-data-dialog-dialog-button-save']"),
    ).toBeVisible();
    // Assertion: Кнопка 'Cancel' (Отмена) отображается в модальном окне
    await expect(
