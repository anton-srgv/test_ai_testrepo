...
/**
 * @externalId 344
 * @recipeId 019ed458-6e7e-708c-a072-dd5e9b4e6a84
 * @title Отображение модального окна при создании Password test data
 * @description Automatically generated test
 */
test(
  'Отображение модального окна при создании Password test data',
  { tag: ['@regression', '@smoke'] },
  async ({ page }) => {
    testit.workItemIds(['344']);
    ...
    // Assertion: На странице отображается блок 'Данные окружения' (Environment Data), подтверждая что это страница редактирования env Data
    ...
    // Step 7: В дропдауне “Data Type“ выбрать “Password“
    // Expected result: Открылось модальное окно с 1. Поле “Name” с описанием под полем: “Password for authentication” 2. Дропдаун: “Data type” с выбранным вариантом: “Password” 3. Поле “Value” с маскированным каким-то значением в виде примера 4. Toggle: “Secret value” с опсианием: “Won't be visible in the fields” и по умолчанию состоянием: on 5. Кнопки: “Save”, “Cancel”
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
    ...
    // Assertion: Под полем Name отображается описание 'Password for authentication' (в локализации — 'Пароль для входа')
    await expect(
      page.locator(
        "[data-testid='env-data-dialog-form'] span:has-text('Пароль для входа')",
      ),
    ).toBeVisible();
    // Assertion: В дропдауне 'Data type' отображается выбранный вариант 'Password' (в локализации — 'Пароль')
    await expect(
      page.locator("[data-testid='env-data-dialog-button-type']"),
    ).toContainText('Пароль');
    // Assertion: Поле 'Value' отображается с маскированным значением-примером
    await expect(
      page.locator("[data-testid='env-data-dialog-input-value']"),
    ).toBeVisible();
    // Assertion: Toggle 'Secret value' по умолчанию включён (состояние on)
    await expect(
      page.locator("[data-testid='env-data-dialog-checkbox-is-secret']"),
    ).toHaveAttribute('data-state', 'checked');
    // Assertion: Описание тогла содержит текст 'Won't be visible in the fields' (в локализации — 'Не будет видно в полях')
    await expect(
      page.locator(
