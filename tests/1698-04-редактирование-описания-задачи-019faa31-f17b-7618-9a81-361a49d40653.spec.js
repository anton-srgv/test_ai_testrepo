...
/**
 * @externalId 2048
 * @recipeId 019faa31-f17b-7618-9a81-361a49d40653
 * @title 1698-04 Редактирование описания задачи
 * @description &lt;p&gt;Нормализовано из #1698. Предусловия: [заранее] Задача SPACECOMPE-1 существует, атрибут «Описание» пустой&lt;&#x2F;p&gt;
 */
test(
  '1698-04 Редактирование описания задачи',
  { tag: ['@normalized', '@src-1698'] },
  async ({ page }) => {
    testit.workItemIds(['2048']);
    ...
    // Assertion: URL содержит путь к канбан-доске проекта, подтверждая вид «Доска»
    await expect(page).toHaveURL(
      ...
      try {
        {
          ...
          try {
            await page
              .locator("[data-class='Button_wrapper']:has-text('Задача')")
              .click();
    ...
    // Assertion: Модальное окно создания задачи открыто и отображается на экране
    await expect(page.locator("[data-testid='modal-header']")).toBeVisible();

    // Step 6: Ввести в поле «Название» значение «Задача_1698_04».
    // Expected result: В поле «Название» отображается «Задача_1698_04».
    // Action: Input variable value into element with data-attribute
    await page.waitForTimeout(300);
    ...
    // Assertion: В поле «Название» отображается введённое значение «Задача_1698_04»
      ...
      try {
        {
          ...
          try {
            const locator = page.locator('div._content_7v048_50');
            ...
            if (apiTriggered) {
              if (
                ...
                await page.locator('div._content_7v048_50').evaluate(
                  (el, timeout) =>
                    new Promise((resolve) => {
                      let done = false;
                      const finish = () => {
    ...
    // Assertion: Кнопка «Создать» активна (не задизейблена) после заполнения обязательных атрибутов типа «Стандарт»
    await expect(
      page.locator(
        "[data-testid='modal'] button[data-testid='button']:has-text('Создать')",
      ),
    ).toBeEnabled();

    // Step 9: Нажать «Создать и перейти в задачу».
    // Expected result: Открыта карточка задачи «Задача_1698_04»; атрибут «Описание» отображает «Не заполнено».
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "[data-testid='modal'] [data-testid='dropdown'] [data-testid='button']",
              )
              .click();
    ...
    // Assertion: Открыта карточка задачи с названием «Задача_1698_04»
    await expect(
      page.locator(
        "[data-testid='modal'] [data-class='TaskTypeHeader_title_2']",
      ),
    ).toContainText('Задача_1698_04');
    // Assertion: Атрибут «Описание» отображает «Не заполнено»
    await expect(
      page.locator(
        "[data-class='AttributeWrapper_wrapper']:has-text('Описание') [data-class='TextEditorPreview_wrapper_2']",
      ),
    ).toContainText('Не заполнено');

    // Step 10: Нажать на значение атрибута «Описание».
    // Expected result: поле «Описание» активно и доступно для ввода.
    // Action: Click on element with css locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "[data-class='AttributeWrapper_wrapper']:has-text('Описание') [data-class='AttributeWrapper_valueContent']",
              )
              .click();
    ...
    // Assertion: Панель инструментов форматирования видна, что подтверждает активность поля «Описание» и его доступность для ввода
    await expect(
      page.locator(
        "[data-class='TextEditor_wrapper'] [data-class='TextEditorMenu_wrapper_2']",
      ),
    ).toBeVisible();

    // Step 11: Ввести в поле «Описание» текст «Описание_1».
    // Expected result: в поле «Описание» отображается «Описание_1».
    // Action: Input variable value into element with css
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await fillInput(
              page,
              "[data-class='AttributeWrapper_wrapper']:has-text('Описание') .tiptap[contenteditable='true']",
              'Описание_1',
            );
    ...
    // Assertion: В поле «Описание» отображается введённый текст «Описание_1»
    await expect(
      page.locator(
        "[data-class='AttributeWrapper_wrapper']:has-text('Описание') .tiptap[contenteditable='true']",
      ),
    ...
    // Step 12: Нажать вне области поля «Описание».
    // Expected result: атрибут «Описание» содержит «Описание_1».
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
    ...
    // Assertion: Атрибут «Описание» содержит текст «Описание_1» после клика вне области поля
    await expect(
      page.locator(
        "[data-class='AttributeWrapper_wrapper']:has-text('Описание') [data-class='TextEditorPreview_wrapper']",
      ),
    ).toContainText('Описание_1');

    // Step 13: Нажать на значение атрибута «Описание» и очистить поле.
    // Expected result: поле «Описание» пустое.
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "[data-class='AttributeTextEditor__hidden']:has-text('Описание_1')",
              )
              .click();
      ...
      try {
        {
          ...
          try {
            await fillInput(
              page,
              "[data-class='AttributeWrapper_wrapper']:has-text('Описание') .tiptap.ProseMirror[contenteditable='true']",
              '',
            );
    ...
    // Assertion: Поле «Описание» пустое после очистки
    await expect(
      page.locator(
        "[data-class='AttributeWrapper_wrapper']:has-text('Описание') .tiptap.ProseMirror[contenteditable='true']",
      ),
    ...
    // Step 14: Нажать вне области поля «Описание».
    // Expected result: атрибут «Описание» отображает «Не заполнено».
    // Action: Click on element with data-attribute locator
    await page.waitForTimeout(300);
    ...
    // Assertion: Атрибут «Описание» отображает текст «Не заполнено» после очистки и клика вне поля
    await expect(
      page.locator(
        "[data-class='AttributeWrapper_wrapper']:has-text('Описание') [data-class='TextEditorPreview_wrapper_2']",
      ),
    ...
    // Step 15: Нажать кебаб-меню (3 точки) в шапке карточки «Задача_1698_04».
      ...
      try {
        {
          ...
          try {
            await page
              .locator(
                "[data-class='TaskTypeHeader_actions'] [data-testid='iconButton']",
              )
              .click();
    ...
    await expect(
      page.locator(
        "[data-testid='popconfirm-card']:has-text('Удалить задачу?')",
      ),
    ).toBeVisible();
    // Assertion: Модальное окно содержит заголовок подтверждения удаления задачи
    await expect(
      page.locator(
        "[data-testid='popconfirm-card-content'] [data-class='PopconfirmCard_title']",
      ),
    ).toContainText('Удалить задачу?');

    // Step 17: Нажать кнопку «Удалить» в модальном окне.
    // Expected result: Задача «Задача_1698_04» отсутствует в списке задач «space-competent-uM3POR».
    // Action: Click on element with css locator
    await page.waitForTimeout(300);
    ...
    // Assertion: Пользователь находится на канбан-доске пространства space-competent-uM3POR после удаления задачи
    await expect(page).toHaveURL(
      new RegExp('\\/project\\/6207\\/view\\/kanban'),
    );
