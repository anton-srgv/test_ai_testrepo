...
/**
 * @externalId 1154
 * @recipeId 019ed4c7-1d3c-777a-886c-33ff31f3aa10
 * @title Успешный поиск тест-кейсов по id при импорте TestIT
 * @description Automatically generated test
 */
test(
  'Успешный поиск тест-кейсов по id при импорте TestIT',
  { tag: ['@regression', '@smoke', '@to-automate'] },
  async ({ page }) => {
    testit.workItemIds(['1154']);
    ...
    // Assertion: Заголовок страницы содержит 'Импортировать из TestIT'
    await expect(
      page.locator("h1:has-text('Импортировать из Test')"),
    ).toBeVisible();
    // Assertion: Кнопка 'Сбросить' всегда видна на странице
    await expect(
    ...
    // Assertion: Кнопка 'Импортировать' всегда видна на странице
    await expect(
    ...
    // Assertion: Сверху над секциями тест-кейсов есть дропдаун с выбором проекта
    await expect(
      page.locator("[data-testid='import-page-sidebar-dropdown']"),
    ).toBeVisible();

    // Step 6: Ввести в поле поиска: “Search” полный/часть siurceId тест-кейса (id)
    // Expected result: Поиск должен возвращать тест кейс(-ы), в случах если введенное значение: * полностью совпадает с id тест кейса * является частью id тест-кейса
    const searchTestCaseId = '115';
      ...
      try {
        {
          ...
          try {
            await fillInput(
              page,
              "[data-testid='import-page-table-section'] input[placeholder='Название, ID или секция']",
              searchTestCaseId,
            );
    ...
    // Assertion: Поле поиска содержит введённое значение searchTestCaseId
    await expect(
      page.locator(
        "[data-testid='import-page-table-section'] input[placeholder='Название, ID или секция']",
      ),
    ).toHaveValue(searchTestCaseId);
    // Assertion: Тест-кейс с ID 1152 отображается в результатах поиска (ID содержит '115' — частичное совпадение)
    await expect(
      page.locator(
        "[data-testid='import-page-table-section'] tr:has(span:text('1152'))",
      ),
    ).toBeVisible();
    // Assertion: Тест-кейс с ID 1156 отображается в результатах поиска (ID содержит '115' — частичное совпадение)
    await expect(
      page.locator(
        "[data-testid='import-page-table-section'] tr:has(span:text('1156'))",
      ),
    ).toBeVisible();
