...
/**
 * @externalId 167
 * @recipeId 019ed52f-ff19-70de-a10f-c34b1407a4d8
 * @title Успешный поиск тест-кейсов по названию при импорте TestIT
 * @description Automatically generated test
 */
test(
  'Успешный поиск тест-кейсов по названию при импорте TestIT',
  { tag: ['@regression', '@smoke', '@to-automate'] },
  async ({ page }) => {
    testit.workItemIds(['167']);
    ...
    // Assertion: Пользователь авторизован — URL изменился на страницу проекта /recipes
    ...
    // Assertion: Заголовок страницы содержит текст 'Импортировать из Test IT'
    await expect(
    ...
    // Assertion: Кнопка «Сбросить» задизейблена при счетчике = 0
    await expect(
      page.locator("[data-testid='import-page-top-bar-button-reset']"),
    ).toBeDisabled();
    ...
    // Assertion: Кнопка «Импортировать» задизейблена при счетчике = 0
    await expect(
      page.locator("[data-testid='import-page-top-bar-button-import']"),
    ).toBeDisabled();
    // Assertion: Дропдаун с выбором проекта виден (значит проектов несколько в интеграции)
    await expect(
      page.locator("[data-testid='import-page-sidebar-dropdown']"),
    ).toBeVisible();

    // Step 6: Ввести в поле: “Search” полное/часть наименования кейса %example
    // Expected result: Поиск должен возвращать тест кейс(-ы), в случах если введенное значение: * полностью совпадает с наименованием тест кейса (регистронезависимо) * является частью наименования тест-кейса * содержит / является спец символом, как и часть наименования тест кейса * english/русский язык использует , etc.
    const searchExample = 'Детальная авторизация';
    ...
    // Assertion: Поле поиска содержит введённое значение [[searchExample]]
    await expect(
      page.locator(
        "[data-testid='import-page-table-section'] input[placeholder='Название, ID или секция']",
      ),
    ).toHaveValue(searchExample);
    // Assertion: В таблице отображается хотя бы один тест-кейс, соответствующий поисковому запросу
    await expect(
      page.locator(
        `[data-testid='import-page-table-section'] table tbody tr:has-text('${searchExample}'):first-child`,
      ),
    ).toBeAttached();
    // Assertion: Название тест-кейса в результатах содержит поисковый запрос — подтверждает, что поиск корректно фильтрует по совпадению с наименованием (полному или частичному, регистронезависимо)
    await expect(
      page.locator(
        "[data-testid='import-page-table-section'] table tbody tr:first-child td:nth-child(5) span",
      ),
    ).toContainText(searchExample);
