...
/**
 * @externalId 1401
 * @recipeId 019e6d42-22fa-753a-9353-350d913efa4c
 * @title Неуспешная авторизация пользователя - неверный пароль
...
test(
  ...
  async ({ page }) => {
    ...
    // Step 2: Заполнить поле password значением wrong_password
    ...
    // Step 3: Заполнить поле email значением test.ai.common.user@gmail.com
    ...
    // Step 4: Нажать кнопку авторизации “Log in“
    // Expected result: Пользователь не авторизован Под полем пароль появляется сообщение об ошибке с текстом: “Invalid credentials“
    ...
    // Assertion: Пользователь не авторизован — остаётся на странице логина
    await expect(page).toHaveURL(new RegExp('\\/login'));
    // Assertion: Под полем пароль появляется сообщение об ошибке (видимость элемента)
    await expect(
      page.locator("[data-testid='login-form'] [role='alert']"),
    ).toBeVisible();
    // Assertion: Сообщение об ошибке содержит текст 'Invalid credentials'
    await expect(
      page.locator("[data-testid='login-form'] [role='alert']"),
    ).toContainText('Неверные учётные данные');
